...
export class CreatorToolsController{
 constructor(private db:PrismaService){}
 @Get('access')
 async access(@CurrentUser()user:any){const[account,global,userSetting]=await Promise.all([this.db.user.findUniqueOrThrow({where:{id:user.sub},select:{credits:true,role:true}}),this.db.adminSetting.findUnique({where:{key:'creator-tools:global'}}),this.db.adminSetting.findUnique({where:{key:`creator-tools:user:${user.sub}`}})]),config={masterEnabled:true,minimumCredits:100,videoTranscript:true,voiceTranscript:true,textToVoice:true,...(global?.value as object)},permissions={videoTranscript:true,voiceTranscript:true,textToVoice:true,...(userSetting?.value as object)},creditEligible=account.role==='ADMIN'||account.credits>=Number(config.minimumCredits),enabled=config.masterEnabled!==false;return{credits:account.credits,minimumCredits:Number(config.minimumCredits),creditEligible,masterEnabled:enabled,permissions,available:{videoTranscript:enabled&&creditEligible&&config.videoTranscript&&permissions.videoTranscript,voiceTranscript:enabled&&creditEligible&&config.voiceTranscript&&permissions.voiceTranscript,textToVoice:enabled&&creditEligible&&config.textToVoice&&permissions.textToVoice}}}
}
...
export class AdminInsightsController{
 ...
 @Get('creator-tools')
 async creatorTools(@CurrentUser()user:any){this.allow(user);const[global,users,settings]=await Promise.all([this.db.adminSetting.findUnique({where:{key:'creator-tools:global'}}),this.db.user.findMany({select:{id:true,fullName:true,email:true,credits:true,role:true},orderBy:{createdAt:'desc'}}),this.db.adminSetting.findMany({where:{key:{startsWith:'creator-tools:user:'}}})]);const access=Object.fromEntries(settings.map(row=>[row.key.slice('creator-tools:user:'.length),row.value]));return{global:{masterEnabled:true,minimumCredits:100,videoTranscript:true,voiceTranscript:true,textToVoice:true,...(global?.value as object)},users:users.map(entry=>({...entry,permissions:{videoTranscript:true,voiceTranscript:true,textToVoice:true,...(access[entry.id] as object)}}))}}
 @Post('creator-tools/global')
 async saveCreatorTools(@CurrentUser()user:any,@Body()body:any){this.allow(user);const value={masterEnabled:body.masterEnabled!==false,minimumCredits:Math.max(0,Number(body.minimumCredits??100)),videoTranscript:body.videoTranscript!==false,voiceTranscript:body.voiceTranscript!==false,textToVoice:body.textToVoice!==false};await this.db.adminSetting.upsert({where:{key:'creator-tools:global'},update:{value,updatedBy:user.sub},create:{key:'creator-tools:global',value,updatedBy:user.sub}});return value}
