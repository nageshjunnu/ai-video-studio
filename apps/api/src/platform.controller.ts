...
export class AdminInsightsController{
 ...
 @Get('creator-tools')
 async creatorTools(@CurrentUser()user:any){this.allow(user);const[global,users,settings]=await Promise.all([this.db.adminSetting.findUnique({where:{key:'creator-tools:global'}}),this.db.user.findMany({select:{id:true,fullName:true,email:true,credits:true,role:true},orderBy:{createdAt:'desc'}}),this.db.adminSetting.findMany({where:{key:{startsWith:'creator-tools:user:'}}})]);const access=Object.fromEntries(settings.map(row=>[row.key.slice('creator-tools:user:'.length),row.value]));return{global:{masterEnabled:true,minimumCredits:100,videoTranscript:true,voiceTranscript:true,textToVoice:true,pixabayImages:true,pexelsImages:true,openverseImages:false,huggingFaceImages:true,geminiVisualPrompts:false,geminiTts:true,relatedVideoClips:false,...(global?.value as object)},users:users.map(entry=>({...entry,permissions:{videoTranscript:true,voiceTranscript:true,textToVoice:true,...(access[entry.id] as object)}}))}}
